...
/*xiaojin-sched-data percpu struct rq*/
...
xiaojin-(exp)原理解释:wake_up_process / try_to_wake_up->感觉让一个进程马上唤醒的步骤：
...
try_to_wake_up(struct task_struct *p, unsigned int state, int wake_flags)
{
	...
	if (p == current) {
		...
		success = 1;
		/*xiaojin-tracepoint 0 trace_sched_waking*/
 ...
 */
/*xiaojin-rcu-synchronize_rcu --5.3  wake_up_process */
...
static void __sched notrace __schedule(bool preempt)
{
	...
	local_irq_disable();
	/*xiaojin-rcu-qs -1 rcu_note_context_switch 一旦进入context_switch就可以说明已经进入QS了
	所以要记录一下*/
