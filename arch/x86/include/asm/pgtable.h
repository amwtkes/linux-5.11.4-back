...
/* xiaojin-mm-page-table-macro pmd_offset -1 pud_page_vaddr 就是取出pud所包含的下一级目录或者pageframe的页的线性地址*/
static inline unsigned long pud_page_vaddr(pud_t pud)
{
	//pud_val(pud) & pud_pfn_mask(pud)应该是mid directory的物理地址
	//pud_val(pud)就是pud.pud
	//pud_pfn_mask(pud) 就是取页表项的掩码（从13-52都是1，其余都是0）
 ...
 * linux/mmzone.h's __section_mem_map_addr() definition:
 */
/*xiaojin-mm-page-table-macro pud_page(pud) pfn_to_page(页号) 获取pud指向下一级目录或者物理地址的页号，并最后返回这个页号对应的page对象。
