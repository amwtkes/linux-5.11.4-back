...
xiaojin-mm-sparsemem-pagetable (exp)原理解释——（如何建立页表映射？）为什么建立页表只要知道线性地址范围即可？具体的函数目的是：为start - end这个线性地址区域建立页表映射。为啥因为在内核，线性地址与物理地址可以简单的通过__pa __va进行转换，是直接映射的{(直接映射区 --- mm.rst ----    ffff888000000000 | -119.5  TB | ffffc87fffffffff |   64 TB | direct mapping of all physical memory (page_offset_base))}，也可以说是连续简单映射的，所以只要一个就知道另一个。所以在这里建立内核页映射只要知道线性地址范围就行了。那为什么要映射呢？因为开启页寻址功能的intel CPU必须通过页表+mmu来寻址，不管用户态还是内核态。映射建立其实就是做好struct page结构去描述被映射的物理内存地址。
*/

/*xiaojin-mm-sparsemem-pagetable -0 vmemmap_populate_basepages调用
*/
		...
		unsigned long nr_pages, int nid, struct vmem_altmap *altmap)
{
	...
	从下面这行代码也可以看到，start就是section的mem_map起始地址，是由vmemmap_base(ffffea0000000000)+pfn得到的(而不是先映射再计算vmemmap！！！)，印证了上面的公式。start是section_memmap的起始页（肯定是页对齐的）的线性地址=pfn（页框号）+ vmemmap。
...
/*xiaojin-mm-sparsemem-pagetable -0(example) 分配内存，只要确定了虚拟地址范围，就可以分配了，大块内存一般是以页为单位分配内存，小块的brk()分配。这里是为struct pages*（也就是这个section的页描述符数组）创建页表映射。start是分配线性地址的起始页位置，end是结束页地址。vmemmap_populate函数进行页表映射。*/
	...
	/*xiaojin-mm-sparsemem-pagetable (exp) !!!struct page的作用：1、在内核中物理地址与虚拟地址转换的计算因子（vmemmap_base + pfn = *page）通过page的虚拟地址可以找到这个page的物理页框的物理地址；知道物理页框的地址可以通过这个公式找到相应的page descriptor虚拟地址，从而知道页的使用情况。2、64个字节存储了这个页的一些属性信息，但是没有包括物理地址哈。__populate_section_memmap本质的作用是！！！===》 为所有物理内存创建页描述符！!!! 
	
	还有啊这里并没有对page进行复制型初始化，毕竟已经获得了page结构的地址，而且后面也不需要再malloc函数去分配page结构，因为已经获得了虚拟地址了，这里就是本质了。对page结构内容清空初始化的地方在这个宏mm_zero_struct_page。
	*/
}
